...
void HunterWrite(std::vector<byte> buffer, bool extrabit)
{
  ...
  // Include an extra 1 bit if requested
  if (extrabit)
  {
    HunterHigh();
  }
...
void HunterStart(byte zone, byte time)
{
  ...
  if (zone < 1 || zone > 48)
  {
    Serial.println("ERROR: Invalid zone number (must be 1-48)");
    return;
  }

  if (time < 0 || time > 240)
  {
    Serial.println("ERROR: Invalid time (must be 0-240 minutes)");
    return;
  }

  // The bus protocol implementation
  ...
  // Disable interrupts during critical timing section for ESP32
  portDISABLE_INTERRUPTS();

  // Write the bits out to the bus
  HunterWrite(buffer, true);

  // Re-enable interrupts
  portENABLE_INTERRUPTS();

  Serial.println("Hunter command sent successfully");
}

/////////////////////////////////////////////////////////////////////////////
// Function: HunterStop
// Description: Stop a specific zone (by setting time to 0)
// Arguments: zone - zone number (1-48)
/////////////////////////////////////////////////////////////////////////////
void HunterStop(byte zone)
{
  ...
  Serial.print(zone);
  Serial.println(" (sending 0-minute command)");
  HunterStart(zone, 0);
}

/////////////////////////////////////////////////////////////////////////////
// Function: HunterProgram
// Description: Run a program
// Arguments: num - program number (1-4)
/////////////////////////////////////////////////////////////////////////////
void HunterProgram(byte num)
{
  Serial.print("HunterProgram: Starting program ");
  Serial.println(num);
  ...
  if (num < 1 || num > 4)
  {
    Serial.println("ERROR: Invalid program number (must be 1-4)");
    return;
  }
  ...
  Serial.println("Hunter program command sent successfully");
}